...
function getWProduct()
{
    ...
    while ($row_products = mysqli_fetch_array($run_products)) {
        ...
        $product_price = number_format($row_products['product_price'], 0, ',', '.');
...
function relatedProducts()
{
    ...
    if (isset($_GET['product_id'])) {
        ...
        while ($row_get_r_products = mysqli_fetch_array($run_get_r_products)) {
            ...
            $p_price = number_format($row_get_r_products['product_price'], 0, ',', '.');
...
function total_price()
{
    ...
    echo "VND " . number_format($total, 0, ',', '.');
}
    ...
    <div class='card col-md-3 col-10' style='margin:0 auto; border-radius:25px 5px;box-shadow: inset -12px -8px 40px #e5e5e5;'>
        ...
        while ($row_items = mysqli_fetch_array($run_itemss)) {
            ...
            while ($row_item = mysqli_fetch_array($run_item)) {
                ...
                $pro_price1 =number_format($pro_price, 0, ',', '.');
                $pro_img1 = $row_item['product_img1'];

                $pro_total_p = $pro_price * $pro_qty;
                $pro_total_p1= number_format($pro_total_p, 0, ',', '.');
            }
        ...
        <tr style='border-bottom: 0.5px solid #ebebeb'>
           ...
           <td class='p-price first-row'>VND $pro_price1</td>
           <td class='qua-col first-row'>
               <div class='quantity'>
                   <div class='pro-qty' data-id='$pro_id'>
                       <input id = 'qqty-$pro_id' type='text' value='$pro_qty'>
                   </div>
               </div>
           </td>
           <td class='total-price first-row' id='total-$pro_id' data-price='$pro_price'>VND $pro_total_p1</td>
...
function checkoutProds()
{
    ...
    if (mysqli_num_rows($run_items) == 0) {
        ...
        while ($row_items = mysqli_fetch_array($run_items)) {
            ...
            echo "
        <li class='fw-normal'>$pro_name x $pro_qty <span>$pro_total_p1</span></li>
